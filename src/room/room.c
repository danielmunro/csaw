...
char *room_to_string(Room *room, Mob *mob, LocationTableT *location_table) {
    ...
    for (int i = 0; i < MAX_MOBS_PER_ROOM; i++) {
        Mob *m = get_mob_by_room_and_index(location_table, room, i);
        if (m && m != mob) {
            strcat(out, get_object_brief(m->description));
            strcat(out, "\n");
        }
    }
    return out;
